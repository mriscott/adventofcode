...
public class Day15{
	...
	gena=new Generator(starta,16807);
	genb=new Generator(startb,48271);
    }

    public int iterate(int num){
	if(num==0) num=40000000;
	int match=0;
	...
	if(!test("short run",65,8921,5,1)) return;
	if(!test("long run", 65,8921,0,588)) return;
	Day15 real = new Day15(783,325);
	int matches=real.iterate(40000000);
...
class Generator{
    ...
    public Generator(long val, long fac){
	value=val;
	factor=fac;
    }
