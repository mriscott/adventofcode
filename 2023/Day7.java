...
public class Day7 extends Utils{
	...
	System.out.println("Part 2: "+real.getWinnings());
	...
	int calcScore(){
		...
		debug(cards +" "+count[0]+"j, score:"+retval);
		if(count[0]==1){
		    switch(retval){
		...
		if(count[0]==2){
		    switch(retval){
		    case 0: //have pair of jokers
			throw new RuntimeException("Prog error");
		    case 1: retval=3; // pair of jokers becomes a 3
			break;
		...
		if(count[0]==4) retval=6;
				    
		debug("New score:"+retval);
	    }
	    score=retval;
	    return retval;
	}
	...
	public String toString(){
	    char [] charry=cards.toCharArray();
	    Arrays.sort(charry);

	    return new String(charry)+"("+getScore()+")";
	}
